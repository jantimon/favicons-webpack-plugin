...
module.exports = function (content) {
  ...
  loadIconsFromDiskCache(self, query, cacheFile, fileHash, function (err, cachedResult) {
    ...
    if (cachedResult) {
      return callback(null, '// LOADER START //' + JSON.stringify(cachedResult) + '// LOADER END //');
    }
    // Generate icons
    generateIcons(self, content, pathPrefix, query, function (err, iconResult) {
      ...
      callback(null, '// LOADER START //' + JSON.stringify(iconResult) + '// LOADER END //');
    });
  });
};
  ...
  fs.exists(resolvedCacheFile, function (exists) {
    ...
    fs.readFile(resolvedCacheFile, function (err, content) {
      ...
      try {
        ...
        if (cache.hash !== fileHash || cache.optionHash !== hashOptions(query) || cache.version !== pluginVersion) {
